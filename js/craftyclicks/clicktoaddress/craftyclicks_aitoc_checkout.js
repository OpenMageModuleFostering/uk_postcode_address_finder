		...
		if ($(fields.postcode_id)) {
			...
			if (_cp_move_fields) {
				...
				if ($(fields.county_id) && $(fields.county_id).up('li') == $(fields.postcode_id).up('li')) {
					...
					// create a new li to hold the country & postcode on its own and put it after the postcode for now
					$(fields.company_id).up('li').insert( {after: '<li class="'+this.li_class+'" id="'+this.prefix+'_cp_postcode_placeholder_id"></li>'} );
					$(this.prefix+"_cp_postcode_placeholder_id").insert( $(this.fields.country_id).up('.field') );
					$(this.prefix+"_cp_postcode_placeholder_id").insert( $(this.fields.postcode_id).up('.field') );
					$(this.fields.town_id).up('.fields').insert( $(this.fields.county_id).up('.field') );
	...
	this.setup_for_uk = function() {
		...
		if ('uk' != this.current_setup) {
			...
			if (!$(this.prefix+'_cp_result_display')) {
				var tmp_html = '<li class="'+this.li_class+' wide" style="display: none"><div class="input-box input-country" id="'+this.prefix+'_cp_result_display">&nbsp;</div></li>';
//				var tmp_html = '<li class="'+this.li_class+'" style="display: none"><div class="one-field" id="'+this.prefix+'_cp_result_display">&nbsp;</div></li>';
			...
			if (!$(this.prefix+'_cp_button_div_id')) {
				...
				$(this.prefix+"_cp_button_id").observe('mouseup', this.button_clicked.bindAsEventListener(this));
			}
			// show button 
		...
		if ('initial' == this.current_setup && _cp_hide_fields) {
			...
			$$('.'+this.prefix+'_cp_address_class').each(function(e){e.hide();});
		}
...
//			alert ('Postcode Lookup could not be added!');
	...
	this.result_ready = function() {
		$(this.prefix+'_cp_result_display').down('select').addClassName('form-control');
		$(this.prefix+'_cp_result_display').up().show();
	}
